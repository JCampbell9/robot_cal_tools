...
rct_image_tools::CircleDetectorParams loadCircleDetectorParams(const YAML::Node& node)
{
  ...
  p.circleColor = static_cast<unsigned char>(getMember<int>(node, "circleColor"));
...
{
  auto rows = config["rows"].as<unsigned>();
  auto cols = config["cols"].as<unsigned>();
...
EXPORT_CLASS_SECTIONED(rct_image_tools::ModifiedCircleGridTargetFinderPlugin, ModifiedCircleGridTargetFinderPlugin,
                       SECTION)
EXPORT_CLASS_SECTIONED(rct_image_tools::CharucoGridTargetFinderPlugin, CharucoGridTargetFinderPlugin, SECTION)
